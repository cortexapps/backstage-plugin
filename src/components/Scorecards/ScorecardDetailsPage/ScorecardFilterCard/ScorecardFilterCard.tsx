...
export const ScorecardFilterCard = ({
  ...
  const [allFilters, setAllFilters] = useState<
    Partial<Record<FilterType, ScorecardServiceScoreFilter>>
  >({});

  const setFilterType = (
    type: FilterType,
    predicate: ScorecardServiceScoreFilter,
  ) => {
    const newFilters = {
      ...allFilters,
      [type]: predicate,
    ...
    <InfoCard title="Filter By" className={classes.root}>
      <ScorecardFilters
        ...
        setFilters={(filters, oneOf) => {
          setFilterType(
            FilterType.FAILING_RULE,
            createRulePredicate(false, filters, oneOf),
          );
        }}
      />
      <ScorecardFilters
        ...
        setFilters={(filters, oneOf) => {
          setFilterType(
            FilterType.FAILING_RULE,
            createRulePredicate(true, filters, oneOf),
          );
        }}
      />
    </InfoCard>
