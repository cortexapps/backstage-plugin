...
import { GroupByOption, ScorecardServiceScore, ScoresByIdentifier } from "../../api/types";
...
describe('<SystemPage />', () => {
  ...
  it('should respect custom scorecard sort order' , async() => {
    const cortexApi: Partial<CortexApi> = {
      ...
      getScorecardScores(_scorecardId: number): Promise<ScorecardServiceScore[]> {
        return Promise.resolve([])
      },

      getServiceScorecardScores(_groupBy: GroupByOption): Promise<ScoresByIdentifier[]> {
        return Promise.resolve([
          {
            identifier: "system:system1",
            scores: [
              { scorePercentage: 0.5, scorecardId: 1, scorecardName: 'Test Scorecard 1' },
              { scorePercentage: 0.5, scorecardId: 2, scorecardName: 'Test Scorecard 2' },
            ],
            numberOfServices: 4,
          },
        ]);
    ...
    const { findByText } = renderWrapped(
      <EntityProvider entity={systemEntity}><SystemPage /></EntityProvider>,
      cortexApi,
      {
        '/': rootRouteRef,
