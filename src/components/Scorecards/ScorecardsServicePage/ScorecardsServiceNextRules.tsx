...
export const ScorecardsServiceNextRules = ({ ladder, score }: ScorecardsServiceNextRulesProps) => {
    const classes = useDetailCardStyles();

    const currentLevel = score.ladderLevels?.[0]?.currentLevel ?? undefined;
    const levels = useMemo(() => [...ladder.levels].sort((a, b) => a.rank - b.rank), [ladder]);
    ...
    const nextLevel = useMemo(() => {
        ...
        const currLevelIndex = levels.findIndex((level) => level.name === currentLevel.name);
        const nextLevelIndex = currLevelIndex + 1;
        return levels.length === nextLevelIndex ? undefined : levels[nextLevelIndex];
    }, [levels, currentLevel]);
                    ...
                    {nextLevel !== undefined ? (
                        <>
                            <Typography variant="subtitle1" className={classes.level}>
                                ...
                                <br/>
                                Complete these rules to get to the next level:<br/>
                                {nextRules.map(rule => <i>&#8226; {rule.title ?? rule.expression}<br/></i>)}
                            </Typography>
                        </>
                    ) : (
                        <Typography variant="subtitle1" className={classes.level}>
