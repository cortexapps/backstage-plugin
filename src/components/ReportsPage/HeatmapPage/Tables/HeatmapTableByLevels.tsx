...
import {getAverageRuleScores, getSortedRulesByLevels, StringIndexable} from "../HeatmapUtils";
...
export const HeatmapTableByLevels = ({
    ...
    if (ladders.length === 0 || ladders?.[0] === undefined) {
        return (
            <WarningPanel
                severity="error"
                title="Scorecard has no ladders."
            />
        );
    }
        ...
        <Table>
            ...
            <TableBody>
                {Object.entries(data).map(([key, values]) => {
                    ...
                    const averageScore = _round(_average(values.map((score) => score.score)), 2);
